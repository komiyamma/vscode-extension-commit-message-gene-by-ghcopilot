...
- コミットメッセージ生成で使用するモデルを `gpt-5` に更新しました。
...
- 既定のイントロ文に「プレーンテキストで記述する」旨を明記し、Markdown表記を使わないよう明確化しました。

## 0.3.13

- 生成されるコミットメッセージでMarkdown表記を避けるよう、既定のイントロ文にプレーンテキスト指示を追加しました。

## 0.3.12

- 生成メッセージが `**` で全体を囲まれている場合、前後の強調マーカーを削除するようにしました。
