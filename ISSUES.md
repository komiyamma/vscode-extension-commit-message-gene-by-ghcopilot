# プロジェクトの課題分析

このドキュメントは、プロジェクトの現状を分析し、確認された課題や改善点をまとめたものです。

---

## 1. アーキテクチャの刷新と残された課題

### 1.1. 現状
本拡張機能は、外部ヘルパー実行ファイルへの依存を撤廃し、`@github/copilot-sdk` を直接利用するモダンなアーキテクチャに刷新されています。これにより、以下の課題が解決されています。

-   **Windows環境への依存:** Node.jsベースになったことで、クロスプラットフォーム対応が実現されました。
-   **メンテナンス性の問題:** TypeScriptに技術スタックが統一され、開発・保守の複雑さが軽減されました。
-   **不安定な出力解析:** SDKが提供するAPIを利用するため、特殊なマーカー文字列に依存する脆い解析処理が不要になりました。

### 1.2. 今後の課題
アーキテクチャの刷新は成功しましたが、それに伴う新たな課題や、以前から引き継がれている改善点が残っています。

---

## 2. 柔軟性に欠ける設定（ハードコードされた値）

### 2.1. 現状
AIモデルの指定やプロンプトの内容など、拡張機能の動作を決定する重要な設定の多くが、`src/extension.ts` 内に直接書き込まれて（ハードコードされて）おり、ユーザーによるカスタマイズができません。

### 2.2. 詳細
-   **AIモデル名:** `model` は `'gpt-5-mini'` に固定されています。ユーザーが異なるモデルを試したい場合、ソースコードの変更が必要です。
-   **AIへの指示（プロンプト）:** コミットメッセージ生成のためのプロンプトが、`buildPrompt` 関数内に文字列として埋め込まれています。ユーザーが独自のスタイルや言語でプロンプトを調整することはできません。
-   **コンテキストの切り詰め長:** `MAX_SECTION_LENGTH` が `3000` 文字に固定されています。より多くのコンテキストをAIに渡したい、あるいは逆に短くして処理を高速化したいといった調整ができません。

### 2.3. 改善案
VSCodeの `settings.json` を通じて、ユーザーが以下の項目を設定できるように機能を追加します。
-   `commitMessageGene.prompt.intro.ja` / `commitMessageGene.prompt.intro.en`: 日本語および英語のプロンプトテンプレート。
-   `commitMessageGene.model`: 使用するAIモデル名。（将来的な追加候補）
-   `commitMessageGene.maxSectionLength`: 各コンテキストセクションの最大文字数。（将来的な追加候補）

---

## 3. エラー処理の改善と今後の課題

### 3.1. 現状と改善点
以前のバージョンでは、API呼び出しの失敗などのエラーメッセージがコミットメッセージ欄に直接書き込まれてしまう問題がありましたが、**この問題は解消されています**。

現在のバージョンでは、`vscode.window.showErrorMessage` APIを利用し、エラーが発生した際には**ユーザーに分かりやすいモーダルダイアログで通知**するよう改善されています。これにより、ユーザー体験が向上しています。

### 3.2. 今後の課題
エラー通知の仕組みは改善されましたが、表示されるメッセージの内容にはまだ改善の余地があります。
-   **技術的なエラーメッセージ:** SDKやGitコマンドから返されたエラーが、しばしばそのまま表示されます。これは技術者以外のユーザーには理解が難しい場合があります。
-   **解決策の不足:** エラーメッセージには、問題の解決に繋がる具体的なアクション（例：「APIキーが設定されているか確認してください」など）が含まれていません。

### 3.3. 改善案
-   エラーの種類を判別し、より具体的で分かりやすいメッセージ（例：「AIからの応答がありませんでした。APIキーの設定やネットワーク接続を確認してください。」）を生成するロジックを導入します。
-   エラー発生時、コミットメッセージ欄は元の状態を維持、または空にするなど、一貫した挙動を保証します。

---

## 4. コンテキスト情報の不足

### 4.1. 現状
`collectGitContext` 関数は `git diff` や `git status` などの基本的な情報を収集しますが、AIがより質の高いコミットメッセージを生成するためには、さらに多くのコンテキストが役立つ可能性があります。

### 4.2. 改善案
収集する情報に、以下のような項目を追加することを検討します。
-   **関連するIssueやタスク:** ブランチ名から `JIRA-123` のようなチケット番号を抽出し、関連情報としてAIに提供する。
-   **コードの静的解析情報:** 変更されたファイルに対してLintや静的解析ツールを実行し、その結果をコンテキストに含めることで、修正の意図をAIがより深く理解できるようにする。（これは高度な機能）
-   **ユーザーによる追記:** コミットメッセージ生成前に、ユーザーが自由記述で補足情報を追加できる入力欄を設ける。

---

## 5. 新たに確認された課題

ソースコードの発展に伴い、以下の新しい課題が確認されました。

### 5.1. マルチリポジトリのワークスペースにおける課題
複数のGitリポジトリが同じワークスペースに開かれている場合、拡張機能は現在アクティブなファイルやVSCodeのUI状態に基づいて、対象のリポジトリを自動的に推測しようとします。

-   **問題点:** この自動選択ロジックが、常にユーザーの意図と一致するとは限りません。意図しないリポジトリのコミットメッセージを生成してしまう可能性があります。
-   **改善案:** 対象リポジトリが明確でない場合、ユーザーにどのリポジトリで操作を実行するかを選択させるためのUI（例: Quick Pick）を提示する。

### 5.2. 国際化対応の拡張性の問題
現在、UIのメッセージやプロンプトの言語は `isJapanese()` という関数で日本語か英語かを判定しています。

-   **問題点:** この実装では、将来的に他の言語（例: ドイツ語、フランス語）を追加する際の拡張性が乏しく、コードの修正が複雑になります。
-   **改善案:** 言語ごとのリソースファイル（例: `package.nls.json`, `package.nls.ja.json`）を利用した、VSCodeの標準的な国際化（i18n）の仕組みを導入する。これにより、言語の追加が容易になり、メンテナンス性も向上します。

これらの課題に対処することで、本拡張機能はより柔軟で使いやすく、堅牢なツールへと進化することができます。
